class BookingDiscount < ApplicationRecord
  ...
  validates :discount_percent, numericality: { greater_than_or_equal_to: 0, less_than_or_equal_to: 100 }, allow_blank: true, if: -> { discount_type == "percent" }
  validates :discount_amount, numericality: { greater_than_or_equal_to: 0 }, allow_blank: true, if: -> { discount_type == "amount" }
  ...
  def at_least_one_discount_present
    if discount_percent.blank? && discount_amount.blank?
      errors.add(:base, "Either discount_percent or discount_amount must be present")
    end
  end
