class RoomReservationsController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to room_reservations_path, status: :see_other, notice: "Room reservation was successfully destroyed." }
  ...
  private
    ...
    def set_room_reservation
      @room_reservation = RoomReservation.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def room_reservation_params
      params.expect(room_reservation: [ :room_id, :booking_id, :check_in_at, :check_out_at, :quantity, :status, :total_price, :note ])
    end
end
