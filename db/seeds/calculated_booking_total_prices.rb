Booking.all.each do |booking|
  room_reservations_price = booking.room_reservations.sum(:total_price)
  ...
  total_price_before_discount = room_reservations_price + additional_services_price + special_requests_price
  ...
  if vip_customer_discounts.present?
    total_percent_discount = vip_customer_discounts.sum(:discount_percent_on_room_price) + vip_customer_discounts.sum(:discount_percent_on_additional_services)
    vip_customer_discounts.each do |discount|
      if discount.percent?
        if discount.discount_percent_on_room_price.present?
          room_reservations_price = room_reservations_price - (room_reservations_price * discount.discount_percent_on_room_price / 100)
        ...
        if discount.discount_amount_on_room_price.present?
          room_reservations_price = room_reservations_price - discount.discount_amount_on_room_price
    ...
    total_price = room_reservations_price + additional_services_price + special_requests_price
  end
