module FilterUtils
  ...
  def filterable_columns
    @filterable_columns ||= begin
      columns = get_index_page_columns(model_class).map do |column|
        column.is_a?(Hash) ? column[:name] : column
      end.uniq.map(&:to_s)

      if model_class.respond_to?(:additional_filterable_columns)
        columns += model_class.additional_filterable_columns
      end
